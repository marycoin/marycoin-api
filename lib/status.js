...
StatusController.prototype.show = function(req, res) {
  ...
  case 'getDifficulty':
    ...
    break;
  case 'getBlockCount':
    this.getBlockCount(function(err, result) {
      if (err) {
        return self.common.handleErrors(err, res);
      }
      res.jsonp(result);
...
StatusController.prototype.getInfo = function(callback) {
  this.node.services.marycoind.getInfo(function(err, result) {
    if (err) {
      return callback(err);
    }
    var info = {
...
StatusController.prototype.getLastBlockHash = function() {
  var hash = this.node.services.marycoind.tiphash;
  return {
    syncTipHash: hash,
    lastblockhash: hash
  };
...
StatusController.prototype.getBestBlockHash = function(callback) {
  this.node.services.marycoind.getBestBlockHash(function(err, hash) {
    if (err) {
      return callback(err);
    }
    callback(null, {
...
StatusController.prototype.getDifficulty = function(callback) {
  this.node.services.marycoind.getInfo(function(err, info) {
    if (err) {
      return callback(err);
    }
    callback(null, {
...
StatusController.prototype.getBlockCount = function(callback) {
  this.node.services.marycoind.getInfo(function(err, info) {
    if (err) {
      return callback(err);
    }
...
StatusController.prototype.sync = function(req, res) {
  ...
  this.node.services.marycoind.isSynced(function(err, synced) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    if (synced) {
    ...
    self.node.services.marycoind.syncPercentage(function(err, percentage) {
      if (err) {
        return self.common.handleErrors(err, res);
      }
      var info = {
        status: status,
        blockChainHeight: self.node.services.marycoind.height,
        syncPercentage: Math.round(percentage),
        height: self.node.services.marycoind.height,
        error: null,
        type: 'bitcore node'
      };
